        ...
        for i in 0..nb_layer {
            ...
            if nb_ngbh > 0usize {
                ...
                for pointwo in &ref_neighbours[i] {
                    neighbours.push(Neighbour::new(pointwo.point_ref.get_origin_id(),pointwo.dist_to_ref,
...
impl LayerGenerator {
    pub fn new(max_nb_connection: usize, maxlevel:usize) -> Self {
        ...
        LayerGenerator{rng: Arc::new(Mutex::new(StdRng::from_entropy())),
            unif: Uniform::<f64>::new(0.,1.), scale:scale, maxlevel:maxlevel}
    }
    //
...
impl<T:Clone+Send+Sync> PointIndexation<T> {
    pub fn new(max_nb_connection: usize, max_layer:usize, max_elements:usize) -> Self {
        ...
        for i in 0..max_layer { // recall that range are right extremeity excluded
            ...
            let frac = (-(i as f64)/(max_layer as f64)).exp() - (-((i+1) as f64)/(max_layer as f64));
        ...
        match entry_point_ref.as_ref() {
            Some(arc_point) => {
                if new_point.p_id.0 > arc_point.p_id.0 {
                    ...
                    log::debug!("\n PointIndexation insert setting max level from {:?} to {:?}", arc_point.p_id.0 , new_point.p_id.0);
...
impl <T:Clone+Send+Sync, D: Distance<T>+Send+Sync > Hnsw<T,D>  {
    ...
    pub fn new(max_nb_connection : usize , max_elements:usize, max_layer:usize, ef_construction: usize, f:D) -> Self {
    ...
    fn set_scale_modification(&mut self, scale_modification : f64) {
        ...
        if scale_modification >= 0.5 && scale_modification <= 2. {
            self.layer_indexed_points.layer_g.set_scale_modification(scale_modification);
            println!(" new scale value {:?}", self.layer_indexed_points.layer_g.scale);
        }
        else {
            println!("\n scale modificationarg {:?} not valid , factor must be between 0.5 and 2.)", scale_modification);
        }
    ...
    fn search_layer(& self, point: &[T], entry_point: Arc<Point<T>> , ef:usize, layer: u8) -> BinaryHeap<Arc<PointWithOrder<T>> > {
        ...
        while candidate_points.len() > 0 {
            ...
            log::trace!("       search_layer, {:?} has  nb neighbours  : {:?} ", c_pid, neighbours_c_l.len());
            for e in neighbours_c_l {
                ...
                if visited_point_id.contains_key(&e.point_ref.p_id) != true {
                    ...
                    if e_dist_to_p < f_dist_to_p || return_points.len() < ef {
                        ...
                        log::trace!("                inserting new candidate {:?}", e_prime.point_ref.p_id);
                        candidate_points.push(Arc::new(PointWithOrder::new(&e.point_ref, -e_dist_to_p)));
        ...
        //
        trace!("return from search_layer, nb points : {:?}", return_points.len());
        return_points
    } // end of search_layer
   ...
   pub fn insert_slice(&self, data_with_id: (&[T],usize))  {
        ...
        let (new_point, point_rank) = self.layer_indexed_points.generate_new_point(data, origin_id);
        ...
        { // I open a read lock on an option
            if let Some(arc_point) = self.layer_indexed_points.entry_point.read().as_ref() {
                ...
                if point_rank == 1 {
                    log::debug!("Hnsw  stored first point , direct return  {:?} ", new_point.p_id);
                    return;
                }
        ...
        for l in ((level+1)..(max_level_observed+1)).rev() {
            ...
            let mut sorted_points = self.search_layer(&data, Arc::clone(enter_point_copy.as_ref().unwrap()), 1, l);
        ...
        for l in (0..level+1).rev() {
            ...
            let mut sorted_points = self.search_layer(&data, Arc::clone(enter_point_copy.as_ref().unwrap()), ef, l);
            log::trace!("in insert :search_layer layer {:?}, returned {:?} points ", l, sorted_points.len());
            sorted_points = from_positive_binaryheap_to_negative_binary_heap(&mut sorted_points);
            if sorted_points.len() > 0 {
                ...
                self.select_neighbours(&data, &mut sorted_points, nb_conn, extend_c, l, keep_pruned, &mut neighbours);
                // sort neighbours
    ...
    fn select_neighbours(&self, data: &[T], candidates: &mut BinaryHeap<Arc<PointWithOrder<T>> >,
        ...
        if extend_candidates {
            ...
            for (_p_id, p_point) in candidates_set.iter() {
                ...
                for q in n_p_layer {
                    if !candidates_set.contains_key(& q.point_ref.p_id) && !new_candidates_set.contains_key(& q.point_ref.p_id) {
        ...
        while candidates.len() > 0 && neighbours_vec.len() < nb_neighbours_asked {
            ...
            if let Some(e_p) = candidates.pop() {
                ...
                if e_to_insert {
                    ...
                    neighbours_vec.push(Arc::new(PointWithOrder::new(&e_p.point_ref, -e_p.dist_to_ref)));
                } else {
                    ...
                    if keep_pruned {
                        discarded_points.push(Arc::new(PointWithOrder::new(&e_p.point_ref, e_p.dist_to_ref)));
                    }
                }
            }
        }
        ...
        if keep_pruned {
            while discarded_points.len() > 0 && neighbours_vec.len() < nb_neighbours_asked {
                ...
                neighbours_vec.push(Arc::new(PointWithOrder::new(&best_point.point_ref, -best_point.dist_to_ref)));
            }
        };
        //
        if log::log_enabled!(log::Level::Trace) {
            log::trace!("exiting select_neighbours : nb candidates: {}", neighbours_vec.len());
    ...
    fn search_with_id(&self, request : (usize, &Vec<T>) , knbn:usize, ef:usize) -> (usize, Vec<Neighbour>) {
    ...
    pub fn parallel_search(&self, datas: &Vec<Vec<T>>, knbn:usize, ef:usize) -> Vec<Vec<Neighbour >> {
        let (sender, receiver) = channel();
        // make up requests
        let nb_request = datas.len();
        let requests : Vec<(usize, &Vec<T>)> = (0..nb_request).into_iter().zip(datas.iter()).collect();
        //
        requests.par_iter().for_each_with(sender,  |s, item| s.send(self.search_with_id(*item, knbn, ef)).unwrap());
...
fn from_negative_binaryheap_to_sorted_vector<T:Send+Sync+Copy>(heap_points : &mut BinaryHeap<Arc<PointWithOrder<T> >> ) -> Vec<Arc<PointWithOrder<T> > > {
    ...
    log::trace!("from_negative_binaryheap_to_sorted_vector nb points in out {:?} {:?} ", nb_points, vec_points.len());
    vec_points
}
...
fn from_positive_binaryheap_to_negative_binary_heap<T:Send+Sync+Clone>(positive_heap : &mut BinaryHeap<Arc<PointWithOrder<T> >> ) -> BinaryHeap<Arc<PointWithOrder<T> > > {
    ...
    log::trace!("from_positive_binaryheap_to_negative_binary_heap nb points in out {:?} {:?} ", nb_points, negative_heap.len());
    negative_heap
}
...
pub(crate) fn check_graph_equality<T1, D1, T2, D2>(hnsw1:&Hnsw<T1,D1>, hnsw2: &Hnsw<T2,D2>)
    ...
    assert!(hnsw1.layer_indexed_points.entry_point.read().is_some() ||
        hnsw1.layer_indexed_points.entry_point.read().is_some(), "one entry point is None");
